...
fn display(pixels: Vec<Vec<u8>>, x: u16, y: u16) { // Display pixels centered at x,y
	let psize = (pixels[0].len() as u16, pixels.len() as u16);
	let origin = (x - psize.0 / 2, y - psize.1 / 2);    
	
	let mut stdout = ::std::io::stdout();
	
	for i in 0..pixels.len() {
		crossterm::execute!(stdout, crossterm::cursor::MoveTo(origin.0, origin.1 + i as u16)).unwrap();
		for j in 0..pixels[i].len() {
			print!("{}", pixels[i][j] as char);
		}
	}
}
...
fn main() {
	...
	opts.optopt("f", "font", "set big font", "PATH");
	...
	loop {
		for word in text.split_whitespace() { // Main loop
			let time: i32 =  300 + 60 * word.len() as i32; // Time to show current word for in milliseconds
